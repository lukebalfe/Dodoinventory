...
Run `supabase-website-fixes.sql` after the made-to-order setup to add item units
and Shopify order staging. A Shopify webhook or Supabase Edge Function should
write pending orders and their lines into `shopify_orders` and
`shopify_order_lines`; the Fulfillment page can then load and fulfill them.
Keep Shopify Admin credentials server-side and never add them to `app.js`.

The same fixes migration also adds `dismissed_at` to purchase orders. Running
the file again is safe: the `if not exists` statements preserve existing data.
The Purchase Orders page uses this field to dismiss pending or completed
orders from the running log without destroying their audit history.
