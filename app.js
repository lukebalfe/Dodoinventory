...
// ---- Image optimisation ----
// Supabase Storage supports on-the-fly image resizing and compression through
// its image transformation API. Instead of downloading a full-resolution photo
// (potentially 2–8 MB from a phone camera) just to display a 54px thumbnail,
// we rewrite the URL to request a pre-shrunk, compressed version — typically
// 5–15 KB instead. The browser caches the transformed version after the first
// load, so subsequent page loads are instant.
//
// Usage:
//   imgUrl(item.image_url, 100)   → 100px wide, quality 75 (thumbnails)
//   imgUrl(item.image_url, 300)   → 300px wide, quality 85 (detail view)
//
// If the URL isn't a Supabase storage URL (e.g. an external link someone
// pasted in), it's returned unchanged so nothing breaks.
function imgUrl(url, width, quality) {
  if (!url) return '';

  // Only rewrite Supabase storage object URLs — leave anything else alone
  if (!url.includes('/storage/v1/object/')) return url;

  // Swap /object/public/ for /render/image/public/ — that's Supabase's
  // image transformation endpoint. Then append size + quality params.
  const transformed = url.replace('/storage/v1/object/', '/storage/v1/render/image/');
  const q = quality || 75;
  const separator = transformed.includes('?') ? '&' : '?';
  return `${transformed}${separator}width=${width}&quality=${q}&resize=contain`;
}
